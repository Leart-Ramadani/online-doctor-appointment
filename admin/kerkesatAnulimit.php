...
<title>Cancelation requests</title>
</head>
...
<body>
    ...
    <div class="d-flex flex-column flex-shrink-0 p-3 text-white bg-dark sidebar">
        ...
        <ul class="nav nav-pills flex-column mb-auto">
            ...
            <li><a href="kerkesatAnulimit.php" class="nav-link text-white active" aria-current="page">Cancelation requests</a></li>
            <li><a href="prices.php" class="nav-link text-white" aria-current="page">Prices</a></li>
    ...
    $countSql = "SELECT COUNT(*) as total FROM kerkesatanulimit";
    ...
    if (isset($_GET['search']) && !empty($_GET['keyword'])) {
        ...
        d.name as 'dep_name' FROM kerkesatanulimit AS k INNER JOIN departamentet AS d ON k.departamenti = d.id LIMIT :startIndex, $entries";
        $prep = $con->prepare($sql);
        $prep->bindValue(':startIndex', $startIndex, PDO::PARAM_INT);
        $prep->execute();
        $data = $prep->fetchAll(PDO::FETCH_ASSOC);
                ...
                <tbody>
                    <?php foreach ($data as $data) : ?>
                        <tr>
                            ...
                            <td>
                                <a class="text-decoration-none text-white" href="tedhenatKerkesave.php?id=<?= $data['id']  ?>" title="Details">
                                    <button class="btn btn-primary p-1 text-white mb-1 rez"><i class="fa-solid fa-circle-info"></i></button>
                                </a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
