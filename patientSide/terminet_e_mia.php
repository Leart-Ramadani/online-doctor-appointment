...
if (isset($_POST['anulo'])) {
    ...
    if (empty($_POST['arsyejaAnulimit'])) {
        ...
        if ($res_prep->execute()) {
            $msg = "Kerkesa juaj per te anuluar terminin eshte parashtruar me sukses. <br>
                    Pergjigjja e kerkeses suaj do t'ju vij ne email.";
            $njoftim = 'njoftim';
        }
    }
}
...
<body>
    ...
    <main class="main mainRes">
        ...
        <?php if ($empty == 'empty') : ?>
            <article style="margin-left: 200px; width: 100%;" class="mt-5">
                <h1 class=" h1 fw-normal text-center mt-5">Nuk ke asnje termin te rezervuar.</h1>
            </article>
        <?php endif; ?>
    </main>
    ...
    <div id="popWindow" class="popAnulo">
        ...
        <div class="emri_pac">
            <p>Emri pacientit: <span><?= $pacienti_fetch['emri'] ?></span></p>
            <hr>
            <p>Mbiemri pacientit: <span><?= $pacienti_fetch['mbiemri'] ?></span></p>
            <hr>
            <p>Email pacientit: <span><?= $pacienti_fetch['email'] ?></span></p>
            <hr>
            <p>Numri personal: <span><?= $pacienti_fetch['numri_personal'] ?></span></p>
            <hr>
            <p>Numri telefonit: <span><?= $pacienti_fetch['telefoni'] ?></span></p>
            <hr>
        </div>
        ...
        <form action="terminet_e_mia.php" method="POST" class="submit_anu">
            <div class="mb-3">
                <label for="arsyeja" class="form-label">Arsyja e anulimit:</label>
